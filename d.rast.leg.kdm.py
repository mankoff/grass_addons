...
#               Ken Mankoff 4/2016: Auto-open wx0, allow multiple maps
#
# PURPOSE:      Displays raster map(s) and a legend on a graphics window.
#
# Description:  d.rast.leg.kdm opens a window if needed, clears the
#               entire screen, divides it into a main (left) and a
#               minor (right) frames, and then display one or more
#               raster maps in the main frame and the map legend of
#               the last-listed map in the minor frame. The user can
#               run the program interactively or non-interactively.
#
...
def main():
    ...
    for m in maps:
        if not grass.find_file(m)['file']:
            grass.fatal(_("Raster map <%s> not found") % m)

    # for rast=
    if rast and not grass.find_file(rast)['file']:
        grass.fatal(_("Raster map <%s> not found") % rast)

    s = grass.read_command("d.mon", flags="p")
    if not s:
    ...
    cats = grass.read_command('r.describe', map=maps[-1], flags='1n')
...
#        lflags += 'n'
    ...
    for m in maps:
        grass.run_command('d.rast', map=m)
        time.sleep(0.25)
